<template>
  <div class="canvas-wrapper" ref="wrapperRef" @drop.prevent="onDrop" @dragover.prevent
    ...
    <svg ref="svgRef" class="flowchart-svg"
      ...
      <rect v-if="showSub2x" width="100%" height="100%" fill="url(#sub2)" />
      <rect v-if="showSub5x" width="100%" height="100%" fill="url(#sub5)" />
      <rect v-if="showSub10x" width="100%" height="100%" fill="url(#sub10)" />

      <g :transform="transformStr">
        <g class="edges-layer">
          <FlowchartEdgeCmp v-for="edge in validEdges" :key="edge.id"
            :edge="edge" :source-node="getNode(edge.sourceNodeId)!" :target-node="getNode(edge.targetNodeId)!"
            :is-selected="edge.id === selectedEdgeId" :all-nodes="edgeRoutingNodes"
            @click="(eid: string) => emit('edgeClick', eid)" />
        </g>
        <g v-if="drawingState?.active" class="drawing-layer">
          <path :d="drawingPath" fill="none" stroke="#4A90D9" stroke-width="2" stroke-linecap="round" stroke-dasharray="6,3" />
          <polygon v-if="drawingState?.active" :points="drawingArrowHead" fill="#4A90D9" />
        </g>
        <g class="nodes-layer">
          <FlowchartNodeCmp v-for="node in nodes" :key="node.id"
            :node="node" :is-selected="node.id === selectedNodeId" :viewport="viewport"
    ...
    <TextNodeEditorCmp v-if="editingInfo"
      :key="editingKey"
      :cx="editingInfo.cx" :cy="editingInfo.cy" :width="editingInfo.width"
      :text="editingInfo.text" :font-size="editingInfo.fontSize" :color="editingInfo.color"
      :viewport="viewport" @commit="(t: string) => emit('labelCommit', t)" @cancel="() => emit('labelCancel')" />
  </div>
</template>
...
function findAnchorNearPoint(nodeId: string, px: number, py: number): AnchorPosition | null {
  ...
  for (const a of ['top','right','bottom','left'] as AnchorPosition[]) {
    const p = getAnchorDisplayPoint(n, a); if (Math.hypot(px-p.x, py-p.y) < 20) return a
  }
  return null
}
